...
export class HotelService {
  ...
  if (location) {
    query.andWhere('hotel.location LIKE :location', { location: `%${location}%` });
  }

    if (rating) {
      query.andWhere('hotel.rating = :rating', { rating });
    }
    if (pricePerNight) {
      query.andWhere('hotel.pricePerNight = :pricePerNight', { pricePerNight });
    }
    if (checkInEndTime) {
